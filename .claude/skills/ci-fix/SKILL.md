...
| `error: Provisioning profile "Together App Store" doesn't include the iCloud capability` / `doesn't include the com.apple.developer.ubiquity-kvstore-identifier entitlement` (archive fails, exit 65) | A new entitlement (e.g. iCloud Key-Value storage in `App.entitlements`) was added to the app, but the existing named provisioning profile predates it — profiles are static snapshots and don't auto-update when the App ID's capabilities change, and Codemagic reuses the existing named profile instead of regenerating it | In Apple Developer Portal: confirm the capability is enabled on the App ID (`com.kricliff.together`) under Identifiers, then delete the stale profile ("Together App Store") under Profiles so Codemagic generates a fresh one on the next build. Regenerating in place also works if that option exists |
